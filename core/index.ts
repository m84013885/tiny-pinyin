...
// 单字拼音
function getPinyin(ch: string, collator?: Intl.Collator): Token {
    if (!collator) {
...
// 多字拼音
function getPinyinForString(input: string, collator?: Intl.Collator): Token[] {
    if (!collator) {
        collator = new Intl.Collator('zh-CN');
    }
    const tokens: Token[] = [];
    for (const ch of input) {
        const token = getPinyin(ch, collator);
        tokens.push(token);
    }
    return tokens;
}
